...
void timeit(system_clock::time_point start, const string & str, int N=0)
{
    ...
    if (N != 0)
    {
        myfile.open("octopus-greedy.txt", ios::app);
        myfile<<N<<" "<<time<<endl;
        myfile.close();
    }
}
...
int main()
{
    int N_arr[5] = {100, 250, 500, 750, 1000}; 
    for (int i = 0; i < 5; i++)
    {
        int N = N_arr[i];
        cout<<"N = "<<N<<endl;
